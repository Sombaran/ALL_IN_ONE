...
#include "app/framework/util/smartron/SM_App_Common.h"
...
{
	syslog(LOG_INFO,"ZB : INIT_CBK-emberAfNcpInitCallback");
}
...
{
	printf("emberAfNetworkKeyUpdateCompleteCallback\n");
}
...
{
	printf("\nDevice_Left : emberAfPluginDeviceTableDeviceLeftCallback\n");
	syslog(LOG_INFO,"ZB : DEV_LEFT_CBK-Device Left Callback");
}
...
{
	printf("\n%sNew Device Callback : emberAfPluginDeviceTableNewDeviceCallback%s\n","\x1B[31m","\x1B[0m");
	syslog(LOG_INFO,"ZB : NEW_DEV_CBK-New Device Callback");
	fflush(stdout);
}
...
{
	syslog(LOG_INFO,"ZB : REJOIN-Device Rejoin");
...
void emberAfPluginDeviceTableStateChangeCallback(EmberNodeId nodeId,
                                                 uint8_t state)
{
	syslog(LOG_DEBUG,"ZB : STST_CHAG_CBK-Device state change callback nodeId=%d,state=%d",nodeId,state);
	printf("\n%semberAfPluginDeviceTableStateChangeCallbacknodeId=%d,state=%d%s\n","\x1B[31m",nodeId,state,"\x1B[0m");
}
...
{
	syslog(LOG_INFO,"ZB : RST_CBK-Pre Ncp Reset Callback");
}
...
{
	printf("\n%sDefault_Poll_Control : emberAfSetDefaultPollControlCallback%s\n\n","\x1B[31m","\x1B[0m");	
}
