...
zwscn_mqueue_t zwscn_mqueue_new(zwscn_mqueue_free_fn_t message_memory_free_handler) {
	return g_async_queue_new_full(message_memory_free_handler);
}

void zwscn_mqueue_delete(zwscn_mqueue_t mqueue_ptr) {
	g_async_queue_unref(mqueue_ptr); //if ref-count reaches 0, the queue is deleted
}

int zwscn_mqueue_length(zwscn_mqueue_t mqueue_ptr) {
	return g_async_queue_length(mqueue_ptr);
}

void zwscn_mqueue_message_push(zwscn_mqueue_t mqueue_ptr, void *message) {
	g_async_queue_push(mqueue_ptr, message);
}
...
void zwscn_mqueue_message_add_by_priority(zwscn_mqueue_t mqueue_ptr, void *message, zwscn_mqueue_sort_fn_t message_priority_sort_handler) {
	 g_async_queue_sort(mqueue_ptr, (GCompareDataFunc)message_priority_sort_handler, NULL);
	 g_async_queue_push_sorted(mqueue_ptr, message, (GCompareDataFunc)message_priority_sort_handler, NULL);
}

void *zwscn_mqueue_message_pop(zwscn_mqueue_t mqueue_ptr) {
	return g_async_queue_pop(mqueue_ptr);
}
...
void zwscn_mqueue_send_exit_message(zwscn_mqueue_t thread_mqueue) {
	...
	if(message ==  NULL) {
 	...
 	message->type = ZWSCN_MESSAGE_TYPE_EXIT;
	message->priority = ZWSCN_MESSAGE_TYPE_EXIT_PRIORITY;
	zwscn_mqueue_message_refcount_inc(message);
	zwscn_mqueue_message_add_by_priority(thread_mqueue, message, zwscn_mqueue_message_priority_sort_handler);
}

void zwscn_mqueue_clear(zwscn_mqueue_t mqueue_ptr) {
	zwscn_mqueue_message_p message;
	
	while((message = g_async_queue_try_pop(mqueue_ptr))) {
