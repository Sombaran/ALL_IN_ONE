...
*/
/**
 * @file    zws_avi_interfaces_sensor_binary.c
 *
...
/** Sensor Binary Setup */
#define COMMAND_SENSOR_BINARY_SETUP              1
/** Sensor Binary Get */
#define COMMAND_SENSOR_BINARY_GET                2
/** Sensor Binary Report */
#define COMMAND_SENSOR_BINARY_REPORT             3
/** Sensor Binary Supported Get */
#define COMMAND_SENSOR_BINARY_SUPPORTED_GET      4
/** Sensor Binary Supported Report */
#define COMMAND_SENSOR_BINARY_SUPPORTED_REPORT   5

/** Sensor Binary type not specified */
 ...
 */
static zws_status_t zws_avi_interfaces_sensor_binary_get_argument(char *(*args)[2], int args_count, avro_value_t *arg_value_p)
{
 ...
 * @retval        ZWS_STATUS_ERROR  Failure
 */
static zws_status_t zws_avi_interfaces_sensor_binary_get_result(int web_api_version, unsigned long combo_id, avro_value_t *arg_value, zwu_string_buffer_t result)
{
	int64_t idle_utime;
	int64_t event_utime;
	...
	if ((zw_avro_field_get_long(arg_value, "idle_utime", &idle_utime) != ZWS_STATUS_OK)
			|| (zw_avro_field_get_long(arg_value, "event_utime", &event_utime) != ZWS_STATUS_OK)
			|| (zw_avro_field_get_int(arg_value, "sensor_value", &sensor_value) != ZWS_STATUS_OK)
	...
	if (zwu_string_buffer_append_format(result,
			"<%s %s=\"%llu\" %s=\"%llu\" state=\"%u\" type=\"%u\" />",
			((web_api_version == ZWS_WEB_API_VERSION_1) ? "bin_snsr" : "bsensor"),
			((web_api_version == ZWS_WEB_API_VERSION_1) ? "idle" : "idle_utime"),
			(uint64_t)idle_utime,
			((web_api_version == ZWS_WEB_API_VERSION_1) ? "event" : "event_utime"),
			(uint64_t)event_utime,
 ...
 * @retval        ZWS_STATUS_ERROR  Failure
 */
static zws_status_t zws_avi_interfaces_sensor_binary_supported_get_result(int web_api_version, unsigned long combo_id, avro_value_t *arg_value, zwu_string_buffer_t result)
{
	int64_t utime;
	...
	ZWS_LOG(ZWS_LOG_ERR, "Failed to get the field");
...
static zws_avi_message_interfaces_t interfaces_sensor_binary_get =
{
		{"zwif_bsensor", "bin_snsr"}, COMMAND_SENSOR_BINARY_GET,
		ZWS_AVI_MESSAGE_PASSIVE_SUPPORTED,
		ZWS_AVI_MESSAGE_SETUP_SUPPORTED,

		"interface_sensor_binary_get",

		zws_avi_interfaces_sensor_binary_get_argument,
		zws_avi_interfaces_sensor_binary_get_result,

		"tServBinSnsr"
};

/** Handlers for Supported Get command */
static zws_avi_message_interfaces_t interfaces_sensor_binary_supported_get =
{
		{"zwif_bsensor", "bin_snsr"}, COMMAND_SENSOR_BINARY_SUPPORTED_GET,
		ZWS_AVI_MESSAGE_PASSIVE_SUPPORTED,
		ZWS_AVI_MESSAGE_SETUP_SUPPORTED,
		...
		NULL,
		zws_avi_interfaces_sensor_binary_supported_get_result,
...
{
	if ((zws_avi_message_list_add_message_interfaces(message_list, &interfaces_sensor_binary_get) != ZWS_STATUS_OK) ||
		(zws_avi_message_list_add_message_interfaces(message_list, &interfaces_sensor_binary_supported_get) != ZWS_STATUS_OK))
	{
