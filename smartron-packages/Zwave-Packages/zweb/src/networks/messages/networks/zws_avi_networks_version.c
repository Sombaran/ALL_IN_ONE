...
*/
/**
 * @file    zws_avi_networks_version.c
 *
 ...
 */
static zws_status_t zws_avi_networks_version_get_result(int web_api_version, avro_value_t *arg_value, zwu_string_buffer_t result)
{
	...
	/* HC API version */
	if (zwu_string_buffer_append_format(result, "<version platform=\"%s\" "
			"app_major=\"%s\" app_minor=\"%s\" ctl_major=\"%u\" ctl_minor=\"%u\" />",
			server_platform,
...
/** Handlers for getting application version information */
static zws_avi_message_networks_t networks_version_get =
{
		{"zw_version", "version"}, ZWS_CGI_NET_SUB_COMMAND_NONE,

		"network_version_get",

		NULL,
		zws_avi_networks_version_get_result,
...
{
	if (zws_avi_message_list_add_message_networks(message_list, &networks_version_get) != ZWS_STATUS_OK)
	{
