...
*/
/**
 * @file    zws_avi_root_network_refresh.c
 *
...
/**
 * @brief Set Avro value for RPC request for Network Refresh command using CGI request arguments
 ...
 */
static zws_status_t zws_avi_root_network_refresh_argument(char *(*args)[2], int args_count, avro_value_t *arg_value_p)
{
	...
	if ((i < args_count) && !strcmp(args[i][0], "network_id")
			&& (zw_avro_field_set_string(arg_value_p, "network_id", args[i][1]) == ZWS_STATUS_OK))
	{
		i++;
...
static zws_avi_message_t root_network_refresh =
{
		"zw_refresh_network",

		"root_network_refresh",

		zws_avi_root_network_refresh_argument,
		NULL,
...
{
	if (zws_avi_message_list_add_message(message_list, &root_network_refresh) != ZWS_STATUS_OK)
	{
