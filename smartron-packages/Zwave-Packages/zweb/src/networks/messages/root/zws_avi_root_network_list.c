...
*/
/**
 * @file    zws_avi_root_network_list.c
 *
...
/**
 * @brief Get CGI response for network list from Avro value for RPC response
 ...
 */
static zws_status_t zws_avi_root_networks_list_result(int web_api_version, avro_value_t *arg_value, zwu_string_buffer_t result)
{
	...
	{
		if (avro_value_get_by_index(&network_list_value, i, &network_value, NULL) != 0)
		{
			goto l_err_get_node_value;
		}

		if (zw_avro_field_get_string(&network_value, "id", &network_id) != ZWS_STATUS_OK)
		{
...
/** Handlers for getting network list */
static zws_avi_message_t root_network_list_get =
{
		"zw_get_network_list",
		...
		NULL,
		zws_avi_root_networks_list_result,
...
{
	if (zws_avi_message_list_add_message(message_list, &root_network_list_get) != ZWS_STATUS_OK)
	{
