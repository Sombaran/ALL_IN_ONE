...
zws_status_t zws_file_name_is_safe(const char *file_name)
{
	...
	while ((ptr = strstr(ptr, "..")))
	{
		...
		{
			ZWS_LOG(ZWS_LOG_ERR, "File name '%s' is unsafe", file_name);
...
zws_status_t zws_read_from_FILE_with_dest_alloc(FILE *file, char **dest_p, int *length, int block_size_hint)
{
	...
	if (!block_size_hint)
	{
		block_size_hint = ZWS_FILE_READ_DEFAULT_BLOCK_SIZE;
	}
	...
	while(1)
	{
		...
		if ((tmp = (char *)ZWS_REALLOC(dest, size)) == NULL)
		{
		...
		ZWS_LOG(ZWS_LOG_DEBUG, "Before: size: %d read: %d bytes: %d", size, read, bytes);
		if ((bytes = fread(dest + read, 1, block_size_hint, file)) != block_size_hint)
		{
			if (ferror(file))
			{
				ZWS_LOG(ZWS_LOG_ERR, "Failed to read file: %s", strerror(errno));
				goto l_err_fread;
			}
			else if (feof(file))
			{
		...
		read += bytes;
		ZWS_LOG(ZWS_LOG_DEBUG, "After : size: %d read: %d bytes: %d", size, read, bytes);
	}

	ZWS_LOG(ZWS_LOG_DEBUG, "Final : size: %d read: %d bytes: %d", size, read, bytes);
	dest[read] = '\0';
...
zws_status_t zws_read_from_file_path_with_dest_alloc(const char *path, char **dest_p, int *length, int block_size_hint)
{
	...
	{
		ZWS_LOG(ZWS_LOG_ERR, "Failed to open file '%s' to read: %s", path, strerror(errno));
		goto l_err_fopen;
	}
	...
	if (!block_size_hint)
	{
		...
		{
			ZWS_LOG(ZWS_LOG_ERR, "Failed to seek file '%s' to the end: %s", path, strerror(errno));
		}
		else
		{
			...
			{
				ZWS_LOG(ZWS_LOG_ERR, "Failed to seek file '%s' back to the top: %s", path, strerror(errno));
				goto l_err_fseek;
			}
		}
	}

	if (zws_read_from_FILE_with_dest_alloc(file, &dest, length, block_size_hint) != ZWS_STATUS_OK)
	{
		goto l_err_read;
	}
	...
	if (fclose(file) != 0)
	{
		ZWS_LOG(ZWS_LOG_ERR, "Failed to close file '%s': %s", path, strerror(errno));
		goto l_err_fclose;
	}
