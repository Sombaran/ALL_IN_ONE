...
#define ZWS_ARRAY_LIST_DEFAULT_ALLOC_COUNT       16
 ...
 */
static zws_status_t zwu_array_list_set_min_capacity(zwu_array_list_t array_list, int min_capacity)
{
	void **tmp;
	int capacity;
	...
	if (min_capacity > array_list->capacity)
	{
		...
		capacity = array_list->capacity + ZWS_ARRAY_LIST_DEFAULT_ALLOC_COUNT;
		if (min_capacity > capacity)
		{
			capacity = min_capacity;
		}

		if ((tmp = (void **)ZWS_REALLOC(array_list->data, sizeof(void *) * capacity)) == NULL)
		{
...
zws_status_t zwu_array_list_add(zwu_array_list_t array_list, void *element)
{
	...
	{
		if (zwu_array_list_set_min_capacity(array_list, array_list->size + 1) != ZWS_STATUS_OK)
		{
...
zws_status_t zwu_array_list_create(int capacity, zwu_array_list_t *array_list_p)
{
	...
	if ((array_list = (zwu_array_list_t)ZWS_MALLOC(sizeof(*array_list))) == NULL)
	{
		ZWS_LOG(ZWS_LOG_ERR, ZWS_MSG_MALLOC_FAILED);
		goto l_err_array_list_alloc;
	}
	...
	if (capacity <= 0)
	{
		capacity = ZWS_ARRAY_LIST_DEFAULT_ALLOC_COUNT;
	}

	if ((array_list->data = (void **)ZWS_MALLOC(sizeof(void *) * capacity)) == NULL)
	{
		ZWS_LOG(ZWS_LOG_ERR, ZWS_MSG_MALLOC_FAILED);
