...
{
	zwp_net_transmit_wait_init((zwp_net_t)zwnet_get_user(desc_interface->net));
}

zwp_status_t zwp_interface_transmit_wait(zwifd_p desc_interface, uint8_t *transmit_status)
{
	return zwp_net_transmit_wait((zwp_net_t)zwnet_get_user(desc_interface->net), transmit_status);
}

void zwp_interface_report_receive_init(int *condition_status_p)
{
	*condition_status_p = ZWP_INTERFACE_REPORT_RECEIVE_WAITING;
}

void zwp_interface_report_receive_done(int *condition_status_p, zwp_thread_cond_t condition_variable)
{
	*condition_status_p = ZWP_INTERFACE_REPORT_RECEIVE_DONE;

	zwp_thread_cond_broadcast(condition_variable);
}

zwp_status_t zwp_interface_report_receive_assertion(int condition_status)
{
	if (condition_status != ZWP_INTERFACE_REPORT_RECEIVE_DONE)
	{
...
{
	return zwp_net_report_receive_wait((zwp_net_t)zwnet_get_user(desc_interface->net), report_receive_condition, timeout);
}
