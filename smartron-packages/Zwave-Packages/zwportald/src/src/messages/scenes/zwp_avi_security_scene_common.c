...
*/
/**
 * @file    zwp_avi_security_scene_common.c
 *
...
zwp_status_t zwp_avi_security_scene_get_last_trigger(avro_value_t *scene_value, const char *avro_name, zwscn_security_scene_last_trigger_p lt) {
	...
	const char *uri;
	uint16_t cmd_cls;
	uint32_t converted_event_id = 0;
	
	if (avro_value_get_by_name(scene_value, avro_name, &lt_value, NULL) != 0) {
		ZWP_LOG(ZWP_LOG_ERR, "Failed to get the field '%s': %s", avro_name, avro_strerror());
		return ZWP_STATUS_ERROR;
	}
	temp_type = lt->type;
	temp_time = lt->time;
	ZWP_LOG(ZWP_LOG_DEBUG, "lt type=%d time=%llu", temp_type, temp_time);
	if ((zw_avro_field_set_int(&lt_value, security_scene_arg_type, temp_type) != ZWP_STATUS_OK)
	        || (zw_avro_field_set_long(&lt_value, security_scene_arg_time, temp_time) != ZWP_STATUS_OK)) {
		return ZWP_STATUS_ERROR;
	}
	if((temp_type == ZWSCN_STATELOG_TRIGGER_EVENT) && (temp_time != 0) && (lt->event != NULL)) {
		//Get URI
		cmd_cls = (uint16_t)(((lt->event->id)) & 0xFFFF);
		...
		converted_event_id = zwp_scn_act_evt_id_2_ifd(lt->event->id);
		if ((zw_avro_field_set_int(&event_value, "event_alarm", (int32_t)lt->event->type) != ZWP_STATUS_OK)
		        || (zw_avro_field_set_int(&event_value, security_scene_arg_status, (int32_t)lt->event->status) != ZWP_STATUS_OK)
		        || (zw_avro_field_set_long(&event_value, security_scene_arg_sutime, (int64_t)lt->event->lrtime) != ZWP_STATUS_OK)
		        || (zw_avro_field_set_string(&event_value, security_scene_arg_uri, (char *)uri) != ZWP_STATUS_OK)
		        || (zw_avro_field_set_int(&event_value, security_scene_arg_cmd, (int32_t)lt->event->cmd) != ZWP_STATUS_OK)
		        || (zw_avro_field_set_int(&event_value, security_scene_arg_ifd, (int32_t)converted_event_id) != ZWP_STATUS_OK)) {
			return ZWP_STATUS_ERROR;
		}
		if (lt->event->device_args) {
			if (zwp_avi_param_to_avro(lt->event->device_args, cmd_cls, lt->event->cmd, &event_value) != ZWP_STATUS_OK) {
				return ZWP_STATUS_ERROR;
			}
		}
	}
