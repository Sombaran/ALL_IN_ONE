...
class AbstractTransport(object):
    ...
    def recv_packet(self,seconds = 0):
        pass
...
class WebsocketTransport(AbstractTransport):
    ...
    def recv_packet(self, rec_seconds=0):
        global last_ping_resp_time
        reconnect_interval = int(rec_seconds + 2)
        try:
            packet_text = self._connection.recv()
        except WebSocketTimeoutException as e:
            current_time = time.time()
            diff_time = (current_time - last_ping_resp_time)

            if diff_time > reconnect_interval:
                raise Exception("missed pong from Server")
            raise TimeoutError('recv timed out (%s)' % e)
        except SSLError as e:
            raise ConnectionError('recv disconnected by SSL (%s)' % e)
        except WebSocketConnectionClosedException as e:
        ...
        engineIO_packet_type, engineIO_packet_data = parse_packet_text(
            packet_text)
        last_ping_resp_time = time.time()
        # if (engineIO_packet_type == 3):
        #    logger2.debug("setting the last time out for ping responce")
        #    last_ping_resp_time = time.time()
...
def get_response(request, *args, **kw):
    try:
        response = request(*args, timeout = 2.5, stream=True, **kw)
