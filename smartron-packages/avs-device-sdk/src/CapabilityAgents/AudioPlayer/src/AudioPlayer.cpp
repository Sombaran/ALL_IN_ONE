...
void AudioPlayer::playNextItem() {
    ...
    /* #21570 : while playing next song the volume is increasing to level 1 automatically when the volume is at level 0.
    this is because, the player is inactive after stop the previous song. But we are assuming the player state will inactive only after completion of playlist
    Hence making player state to active before playing next song*/ 
...
void AudioPlayer::pauseMedia() {
    ACSDK_DEBUG1(LX("pauseMedia"));
    m_mediaPlayer->pause(m_sourceId);
}
 /**
 * BUG:21972
 *  Adding stop media to stop the MUSIC content.
 */
void AudioPlayer::stopMedia() {
    ACSDK_DEBUG1(LX("stopMedia"));
    m_mediaPlayer->stop(m_sourceId);
}

void AudioPlayer::playMedia() {
    ACSDK_DEBUG1(LX("playMedia"));
	playNextItem();
}

bool AudioPlayer::isMediaPlayerActive() {
	ACSDK_DEBUG1(LX("isMediaPlayerActive"));
	return m_mediaPlayer->isMediaActive(m_sourceId);
}
/**
 * Bug #21360 : End
 * */
