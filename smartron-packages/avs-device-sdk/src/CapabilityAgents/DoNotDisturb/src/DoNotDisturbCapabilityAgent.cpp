...
std::shared_ptr<DoNotDisturbCapabilityAgent> DoNotDisturbCapabilityAgent::create(
    ...
    std::shared_ptr<avsCommon::sdkInterfaces::FocusManagerInterface> focus) {
    if (!messageSender) {
    ...
    /* #21139: Adding FocusManager Interface object */
    if (!focus) {
        ACSDK_ERROR(LX("createFailed").d("reason", "focusNull"));
        return nullptr;
    }

    /* #21139: Adding FocusManager Interface object */
    auto dndCA = std::shared_ptr<DoNotDisturbCapabilityAgent>(new DoNotDisturbCapabilityAgent(
        customerDataManager, exceptionEncounteredSender, messageSender, settingsManager, focus));
...
std::shared_future<MessageRequestObserverInterface::Status> DoNotDisturbCapabilityAgent::sendDNDEvent(
    ...
    if(value == "true")
    {
        bool channel;
        Dnd_state = 1;
        ACSDK_DEBUG9(LX("DND enabled"));
        channel = m_focusManager->channelState();
        if(channel == false)
        {
