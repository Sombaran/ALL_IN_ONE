...
#include <chrono>
// Redmine 21156: Start: Syslog header file
#include <syslog.h>
// Redmine 21156: End
...
// Redmine 21823: Start: Added for sleep
#include <unistd.h>
// Redmine 21823: End

// Redmine 21425: Start: Maximum Volume of Pulseaudio
#define PA_MAX_VOLUME 127
// Alsa Sink Card Name
#define ALSA_SINK_CARD "alsa_output.0 "
// Redmine 21425: End

// Redmine 21156: Start: Declaration of Wakeword Detection and Cloud_playback status variables
// Whenever wd detects it will call
extern bool WD_BT_trig;

// Declaration and definition for WD variable 
// Redmine 21823: Start: Changed to extern to read in Device Manager
extern bool WD_trig_internal;
// Redmine 21823: End

// Declaration and definition for cloud_play variable
// Redmine 21463: Start: Variable to Access from BluetoothDeviceManager
extern bool cloud_play;
// Redmine 21463: End
// Redmine 21156: End
extern bool external_play;

// Redmine 21463: Start: Variable will be Triggered when Alarm will Go to OFF state
// Redmine 22060: changed type to support ALARM_STARTED
...
/// Redmine 22060: alarm_trig will be moved to IDLE state once the Alert is stopped
static const uint8_t ALERT_IDLE = 0;
...
// Recording the Last volume when Wakeword or Alarm is triggered, initial value is set to Maximum
static uint16_t lastVolume=PA_MAX_VOLUME;
...
bool Bluetooth::init() {
    ...
    m_eventBus->addListener(
        {avsCommon::utils::bluetooth::BluetoothEventType::DEVICE_DISCOVERED,
         ...
         avsCommon::utils::bluetooth::BluetoothEventType::SOURCE_VOLUME_RECEIVED},
        shared_from_this());
...
void Bluetooth::executeEnterForeground() {
    ...
    syslog(LOG_DEBUG, "[SHIFU][BT]:%s: m_streamingState = %s",__func__, streamingStateToString(m_streamingState).c_str());
    if (!m_activeDevice) {
        ACSDK_ERROR(LX(__func__).d("reason", "noActiveDevice"));
        // Redmine 21298: Start: Once the Device is disconnected, Setting all triggers to false State
        WD_trig_internal=false;
        cloud_play = false;
        // // Redmine 21425: Start: External play event is just to verify and not to be assigned
        // external_play = false;
        // // Redmine 21425: End
        // Redmine 21463: Start: After BT disconnection Variable will be moved to Pause State 
        alarm_trig = ALERT_IDLE;
        // Redmine 21463: End
        // Redmine 21298: End
        // Redmine 21425: Start: After Disconnection Volume will set back to Maximum
        executeVolumeChanged((uint16_t) PA_MAX_VOLUME);
        lastVolume = PA_MAX_VOLUME;
        // Redmine 21425: End
    ...
    switch (m_streamingState) {
        ...
        case StreamingState::PENDING_ACTIVE:
            // // Redmine 22058: Start: Should be Turned OFF after Setting MediaPlayer Active Status.
            // WD_trig_internal=false;
            // cloud_play = false;
            // alarm_trig = false;
            // // Redmine 22058: End
            ...
            executeDrainQueue();
            // Redmine 21425: Start: when Play is initiated, it plays with the Last assigned volume in iOS phone
            executeVolumeChanged(lastVolume);
            // Redmine 21425: End
            // Redmine 22058: Start: If Alarm is Stopped, MediaPlayer should play with Speaker volume
            if(!alarm_trig)
            // Redmine 22058: End
                // Redmine 21938: Start: When IsPlayerActive is True, Mediaplayer won't trigger the Volume
                m_mediaPlayer->IsPlayerActive = true;
                // Redmine 21938: End
            // Redmine 22058: Start: After Setting Player Active based on triggers, Triggers will reset
            WD_trig_internal=false;
            cloud_play = false;
            alarm_trig = ALERT_IDLE;
            // Redmine 22058: End
        ...
        case StreamingState::INACTIVE:
            ...
            // Redmine 21156: Start: Conditional check to invoke play based on status of WD, Cloud playback and shifu stream state
            // external_play will trigger the AVRCP Play operation is it is paused.
            // Redmine 21425: Start: when Play is initiated, it plays with the Last assigned volume in iOS phone
            executeVolumeChanged(lastVolume);
            // Redmine 21425: End
            syslog(LOG_DEBUG,"[SHIFU][BT]:%s: Out: WD_trig_internal = %d, cloud_play = %d ,alarm_trig = %d, external_play = %d\n",__func__,WD_trig_internal,cloud_play,alarm_trig,external_play);
            if(WD_trig_internal || cloud_play || alarm_trig || external_play)
            {
                // Redmine 21823: Start: Sleep will ignore IDLE transition state
                sleep(1);
                // Redmine 21823: End
            ...
            // alarm_trig = false;
            // // Redmine 22058: End
            syslog(LOG_DEBUG,"[SHIFU][BT]:%s: Trigs are zeros\n",__func__);
            // Redmine 21298: Start: When External Play initiated From Android/iOS phone.
            // // Redmine 21425: Start: External play event is just to verify and not to be assigned
            // external_play = false;
            // // Redmine 21425: End
            // Redmine 21156: End
...
void Bluetooth::executeEnterBackground() {
    ...
    if (!m_activeDevice) {
        ...
        // external_play = false;
        // // Redmine 21425: End
        // Redmine 21463: Start: After Disconnection Alarm_trig will be disabled
        alarm_trig = ALERT_IDLE;
        // Redmine 21463: End
        // Redmine 21298: End
        // Redmine 21425: Start: After Disconnection Volume will set back to Maximum
    ...
    switch (m_streamingState) {
        case StreamingState::ACTIVE:
            syslog(LOG_DEBUG,"[SHIFU][BT]:%s: WD_BT_trig = %d",__func__,WD_BT_trig);
            // Redmine 21298: Start: Differentiation between Mediaplayer and Mediatransport, don't have Wakeword dependencies.
            // Redmine 21463: Start: Wakeword interface Recorded and used to initialised after Wakeword Response
            if(WD_BT_trig)
                WD_trig_internal=true;
            // Redmine 21463: End
            // Redmine 21298: End
            ...
            // Redmine 21425: Start: when the Wakeword/Alarm is detected, then it should play with Maximum Volume
            executeVolumeChanged((uint16_t) PA_MAX_VOLUME);
            // Redmine 21425: End
            ///< #21354 For AVRCP pause we are sending mediaplayer stop. So from AVRCP prespective making player as in active state.
            // Redmine 21938: Start: Disabling this as there are other functions which are making it to go FALSE state.
            // m_mediaPlayer->IsPlayerActive = true;
            // Redmine 21938: End
            break;
        ...
        case StreamingState::PENDING_PAUSED:
        case StreamingState::INACTIVE:
            // Redmine 21425: State: When Paused/PendingPause/Inactive making sure that Everything is in Pause State
            if(WD_BT_trig)
                WD_trig_internal=true;
            // Redmine 21463: End
            // Redmine 21298: End
...
// Redmine 21319: Start: Disabling Focus Manager to Enter into NONE, Instead of NONE, BACKGROUND will be processed.
...
void Bluetooth::handleDirective(std::shared_ptr<CapabilityAgent::DirectiveInfo> info) {
    ...
    m_executor.submit([this, info] {
        ...
        if (directiveName == SCAN_DEVICES.name) {
            ...
            // Redmine 21463: Start: cloud_play is triggered when Cloud Play, next and previous is initiated
            cloud_play = true;
            // Redmine 21463: End
            executePlay();
        } else if (directiveName == STOP.name) {
            // Stop voice command case Should not trigger Play.
            cloud_play=false;
            WD_trig_internal=false;
            executeStop();
            ...
            // Redmine 21823: Start: Since IDLE case is ignored with Wakeword, Extra play option is not required
            // executePlay();
            // Redmine 21823: End
            ...
            // Redmine 21463: Start: Considering Looping scenario, Play case is initiated if Previous is pause with Focus Manager operations
            executePlay();
            // Redmine 21463: End
        } else {
...
void Bluetooth::executePlay() {
    ACSDK_DEBUG5(LX(__func__));
    // Redmine 21156: Start: Defining the cloud_play variable when play issued.    
    cloud_play = true;
    // Redmine 21156: End
    if (!m_activeDevice) {
...
// Redmine 21425: Start: Callback to set Volume Changes
void Bluetooth::executeVolumeChanged(uint16_t volume) {
    ACSDK_DEBUG5(LX(__func__).d("Volume", volume));
    std::string cmd;
    int8_t volume_int=volume/1.27;
    cmd = "pactl set-sink-volume " ALSA_SINK_CARD + std::to_string(volume_int) + "\%";
    system(cmd.c_str());
}
// Redmine 21425: End
...
void Bluetooth::executeOnDeviceConnect(std::shared_ptr<BluetoothDeviceInterface> device) {
    ...
    // // Redmine 21710: Start: 
    // // Disabling disconnect to overcome self disconnect, 
    // // Case will be handled with shifu_last_bt_disconnect() callback
...
void Bluetooth::onEventFired(const avsCommon::utils::bluetooth::BluetoothEvent& event) {
    ...
    switch (event.getType()) {
                ...
                case avsCommon::sdkInterfaces::bluetooth::DeviceState::UNPAIRED: {
                    m_executor.submit([this, device] {
                        if (device == m_activeDevice) {
                            // Redmine 21425: Start: Once Device is disconnected, ALSA volume is restored to default
                            executeVolumeChanged(PA_MAX_VOLUME);
                            lastVolume = PA_MAX_VOLUME;
                            external_play = false;
                            // Redmine 21425: End
                            executeOnDeviceDisconnect(Requester::DEVICE);
                ...
                case avsCommon::sdkInterfaces::bluetooth::DeviceState::CONNECTED: {
                    m_executor.submit([this, device] {
                         ...
                         */
                            ...
                            executeOnDeviceConnect(device);
                            // Redmine 21425: Start: Once Device is Connected, ALSA volume is restored to default to have a Volume Synchronisation
                            executeVolumeChanged(PA_MAX_VOLUME);
                            lastVolume = PA_MAX_VOLUME;
                            // Redmine 21425: End
                            /*
        ...
        case avsCommon::utils::bluetooth::BluetoothEventType::STREAMING_STATE_CHANGED: {
            ...
            if (A2DPRole::SINK == *event.getA2DPRole()) {
                if (MediaStreamingState::ACTIVE == event.getMediaStreamingState()) {
                    ...
                    m_executor.submit([this] {
						syslog(LOG_DEBUG, "[SHIFU][BT]:%s:  Before FocusAcquire m_streamingState = %s",__func__, streamingStateToString(m_streamingState).c_str());
        ...
        // Redmine 21425: Start: On Volume Event Fired, Volume will set if playing, or recorded if paused.
        case avsCommon::utils::bluetooth::BluetoothEventType::SOURCE_VOLUME_RECEIVED: {
            lastVolume = event.get_shifu_source_volume();
            if(external_play)
             	executeVolumeChanged(lastVolume);
            break;
        }
