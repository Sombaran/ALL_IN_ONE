...
bool isAVSinInitFirstTime = false;
std::mutex avs_config;
// #22267 End.

// Redmine 22060: Changing type for variable to support reading ALERT_START
...
/// Redmine 22060: Triggered when Alert Started 
static const uint8_t ALERT_STARTED = 2;

/// Redmine 22060: Triggered when Alert Stopped 
static const uint8_t ALERT_STOPPED = 1;
...
bool AlertsCapabilityAgent::initialize() {
	...
	/* #22115: Updating previous alerts volume to AVS to have sink between device and avs cloud */
	updateAVSWithLocalVolumeChanges(m_lastReportedSpeakerSettings.volume, true);
...
void AlertsCapabilityAgent::executeOnFocusManagerFocusChanged(
    ...
    if (m_alertIsSounding) {
        if (!m_commsChannelIsActive && !m_contentChannelIsActive) {
            ...
            /* #21611: While playing Alarm, without interrupt Volume LED has observed
            * commenting set volume, because we are setting volume when starting of playback*/
...
void AlertsCapabilityAgent::executeOnAlertStateChange(
	...
	if(isAVSinInitFirstTime == false){
		...
		ACSDK_DEBUG9(LX("executeOnAlertStateChange").d("info", "Sending AVS configured to LED module"));
    ...
    switch (state) {
        ...
        case AlertObserverInterface::State::STARTED:
        	/* #21128: sending start alert pattern to led server as the alert playback started */
        	alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_ALARM, 1, 0);
            // Redmine 21463: Start: As IDLE State Updation is remove, After Alarm trigger will be performed to initialise the BT stream
            alarm_trig = ALERT_STARTED;
            // Redmine 21463: End
        ...
        case AlertObserverInterface::State::STOPPED:
        	/* #21128: sending stop alert pattern to led server as the alert playback stopped */
            // Redmine 21463: Start: As IDLE State Updation is remove, After Alarm trigger will be performed to initialise the BT stream
            alarm_trig = ALERT_STOPPED;
            // Redmine 21463: End
        	alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_ALARM, 0, 0);
        ...
        case AlertObserverInterface::State::COMPLETED:
        	/* #21128: sending stop alert pattern to led server as the alert playback completed */
            // Redmine 21463: Start: As IDLE State Updation is remove, After Alarm trigger will be performed to initialise the BT stream
            alarm_trig = ALERT_STOPPED;
            // Redmine 21463: End
        	alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_ALARM, 0, 0);
    ...
    if (alertIsActive) {
        ...
        if (m_alertIsSounding) {
            ...
            m_alertIsSounding = false;
            /**  Redmine :21181
             * Fix:  As avs device sdk uses gstreamer sessions to address the volumes but smartron uses device volume for volume change so no need to reset the volume.
             */
            /**m_speakerManager->setVolume(
              *  SpeakerInterface::Type::AVS_ALERTS_VOLUME, m_lastReportedSpeakerSettings.volume);
              */
...
void AlertsCapabilityAgent::setNextAlertVolume(int64_t volume) {
	...
	/* #21219
	* change volume for present alert
	*/
    m_speakerManager
...
void AlertsCapabilityAgent::executeOnSpeakerSettingsChanged(
    ...
    /*#21805:Updating the local volume change to AVS irrespective of the alert state */
    if (SpeakerInterface::Type::AVS_ALERTS_VOLUME == type) {
