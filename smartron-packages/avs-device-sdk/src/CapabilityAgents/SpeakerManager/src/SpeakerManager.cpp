...
/* #22273: Adding FocusManager Interface object */
std::shared_ptr<SpeakerManager> SpeakerManager::create(
    ...
    auto speakerManager = std::shared_ptr<SpeakerManager>(
        new SpeakerManager(speakers, contextManager, messageSender, exceptionEncounteredSender, focusManager));
    ...
    if (directiveName == SET_VOLUME.name) {
        ...
        if (jsonUtils::retrieveValue(payload, VOLUME_KEY, &volume) &&
            ...
            m_executor.submit([this, volume, directiveType, info] {
				...
				/* #21329
				* removed execute set mute. This mute is dependency of gstreamer volume control. 
				* We are using hardware volume. So we dont have mute dependency.	
				*/
                if (executeSetVolume(
                        directiveType,
...
/** #21113: The below lines are modified by smartron. 							*
* changes done to implement volume control through hardware buttons 			*
* Alexa sdk is controlling volume of gstreamer(stream volume). 
* It has 5 gstreamer sessions and controlling volume 5 times using for loop 	*
* But smartron implementation is controlling the device volume.					* 
* Hence controls the volume only once. Hence commented the for loop. 			*
*/
...
*/
    ...
    //}
        ...
        if(source == SpeakerManagerObserverInterface::Source::LOCAL_API)
        {
            bool channel = m_focusManager->channelState();
            if(channel == false)
            {
                if(access( "/tmp/avs_volume.lock" , F_OK ) == -1)
                {
                    system("touch /tmp/avs_volume.lock && aplay -f S16_LE -r 48000 /wav_files/volume.wav && rm /tmp/avs_volume.lock &");
                }               
            }
        }
