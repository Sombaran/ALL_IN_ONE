...
// Redmine 21156: Start: Header file    
...
// Redmine 21156: Start: Music is reasumed after getting the message notification    
...
using namespace alexaClientSDK::avsCommon::utils::led;
                ...
                case AuthObserverInterface::State::UNRECOVERABLE_ERROR:
                    switch (newError) {
                        ...
                        case AuthObserverInterface::Error::INVALID_CODE_PAIR:
                            ...
                            /* #21477: run the deregister script for
                            	1. Sending signal to mobile agent
                            	2. stop AVS(VA) service
                            	3. Remove AVS db files
                            	4. Start AVS service
                            */
                            syslog(LOG_DEBUG, "ALEXA: Device deregistered from alexa app");
                            /* #21704: 
                            * Alerm pattern is keeps playing if deregister the device while playing alarm. *
                            * So that turning off all possible patterns if they are in the queue/playing *
                            */
                            alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_ALARM, 0, 0);
                            alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_RESPONSE, 0, 0);
                            alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_PROCESSING, 0, 0);
                            alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD, 0, 0);
                            alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_ANNOUNCEMENT, 1, LED_AUTH_FAIL);
                            system("sh /root/va_data/alexa/deregister.sh");
                            break;
                    }
                    break;
            }
        ...
        switch (m_dialogState) {
            case DialogUXState::IDLE:
				...
				if(isAVSinInitFirstTime == false){
					...
					ConsolePrinter::prettyPrint("Sending AVS configured to LED module");
					isAVSinInitFirstTime = true;
				}
				avs_config.unlock();
				// #22267 End.
                // Redmine 21463: Start: Changes for handling Alarm BT source stream scenario
                WD_BT_trig = false;
                // Redmine 21463: End
                /* api to send led pattern to led-sever*/
                /* turning off all wakeword patterns as alexa is in idle state */
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_RESPONSE, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_PROCESSING, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD, 0, 0);
                ConsolePrinter::prettyPrint("Alexa is currently idle!");
                /* #21139: start: shifu-alexa to display the purple LED for "Do Not Disturb off" disable/enable*/
                if(Dnd_state == 1)
                {
                    ConsolePrinter::prettyPrint("DND led triggered");
                    alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_DND, 1, 0);

                }
#ifdef AUTO_TEST_SUPPORTED
                system("uci set avs_autotest.AVS_STATUS=0");
#endif
                return;
            case DialogUXState::LISTENING:
                /* Invoking wakeword led pattern as alexa is listening to user */
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_RESPONSE, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_PROCESSING, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD, 1, 0);
	            alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD_OFF, 1, 0);
                ConsolePrinter::prettyPrint("Listening...");
		        // Redmine 21156: Start: Assinging WD_BT_trig with value whenever the Wakeword is triggered     
                WD_BT_trig=true;
		        // Redmine 21156: End
#ifdef AUTO_TEST_SUPPORTED
                system("uci set avs_autotest.AVS_STATUS=1");
#endif
                return;
            case DialogUXState::EXPECTING:
                ConsolePrinter::prettyPrint("Expecting...");
#ifdef AUTO_TEST_SUPPORTED
                system("uci set avs_autotest.AVS_STATUS=4");
#endif
                return;
            case DialogUXState::THINKING:
                /* Invoking query processing led pattern as alexa is receiving answer from cloud */
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_RESPONSE, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_PROCESSING, 1, 0);
                ConsolePrinter::prettyPrint("Thinking...");
    		      // Redmine 21156: Start: Assinging WD_BT_trig with value whenever the post Wakeword detected.  
                WD_BT_trig=false;
		          // Redmine 21156: End  
#ifdef AUTO_TEST_SUPPORTED
                system("uci set avs_autotest.AVS_STATUS=2");
#endif
		return;
            case DialogUXState::SPEAKING:

                /* Invoking query response led pattern as alexa is responding to user */
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_PROCESSING, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_RESPONSE, 1, 0);
	           // Redmine 21263: Start: Trigger End Speaking LED pattern as LED_WAKEWORD_OFF
	           alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD_OFF, 1, 0);
	           // Redmine 21263: End
                ...
                // Redmine 21463: End
#ifdef AUTO_TEST_SUPPORTED
                system("uci set avs_autotest.AVS_STATUS=3");
#endif
                return;
            /*
            ...
            case DialogUXState::FINISHED:
                ...
                WD_BT_trig = false;
                // Redmine 21463: End
                /* turn off all wakeword related patterns as user intercation is finished */
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_PROCESSING, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_WAKEWORD, 0, 0);
                alexaClientSDK::avsCommon::utils::led::send_pattern_to_led_server(LED_QUERY_RESPONSE, 0, 0);
                return;
