...
/* #21158: 	the below microphone on/off api's written by smartron based on  microphoneToggle() api*/
/* #21158: 	This function indicates to AVS as the microphone is in on state. */
void InteractionManager::microphoneOn() {
    m_executor.submit([this]() {
        if (!m_wakeWordAudioProvider) {
            return;
        }
		
		syslog(LOG_DEBUG, "ALEXA: microphone turned On");
        m_isMicOn = true;
        m_micWrapper->startStreamingMicrophoneData();
        m_userInterface->microphoneOn();
        });
...
/* #21158: 	This function indicates to AVS as the microphone is in off state. */
void InteractionManager::microphoneOff() {
    ...
    m_executor.submit([this]() {
		...
		syslog(LOG_DEBUG, "ALEXA: microphone turned Off");
        m_isMicOn = false;
        m_micWrapper->stopStreamingMicrophoneData();
        m_userInterface->microphoneOff();
        });
 ...
 * */
void InteractionManager::buttonActive() {
    m_executor.submit([this]() {
        m_client->notifyOfActionButton(m_tapToTalkAudioProvider, m_isMicOn).get();
    });
}
/**
 * Feature #21131 : End
...
/* #21219
* get the priority value of active channel running
*/
unsigned int InteractionManager::getActiveChannelPriority()
{
	return m_client->getActiveChannelPriority();
}
