...
/* header file for led patterns */
#include "shifu-led.h"
/* led client api's header file */
...
bool isDeviceRegisteredWithAVS = false;
// Redmine 21542: End.
...
/* zmq receive buffer size. This is used to receive data from Mobile agent */
#define LWA_ZMQ_BUF_SIZE	4096

/* authentication code expiration time */
#define LWA_AUTH_CODE_EXPIRE_DURATION	60
...
/**************************** #20994 start: adding variables for app based registration ***********************/

/// request header code used during the LWA token request.
static const std::string POST_VALUE_AUTH_CODE = "authorization_code";

/// key for auth code used during the LWA token request.
static const std::string POST_KEY_AUTH_CODE = "code";

/// Key for redirect uri used during the LWA token request.
static const std::string POST_KEY_REDIRECT_URI = "redirect_uri";

/// key for code verifier used during the LWA token request.
static const std::string POST_KEY_CODE_VERIFIER = "code_verifier";

/**************************** #20994 End: adding varibles for app based registration ***********************/
...
CBLAuthDelegate::FlowState CBLAuthDelegate::handleStarting() {
    ...
    if (m_storage->getRefreshToken(&m_refreshToken)) {
   		/* #22313 setting NV parameter for error handlings */
    	alexaClientSDK::avsCommon::utils::uci::SetparamUCI("shifu.conf", "ALEXA_REGISTRATION_STATUS", "registered");
		isDeviceRegisteredWithAVS = true;
...
/**
 * #20994: This function definition is changed by smartron for app based registration
 * handleRequestingCodePair() - creates a ZMQ context. Waits in ZMQ receive to get the auth code from mobile agent. (mobile agent receives this auth code from mobile app).
 * @return 	returns the next action to perform in AVS.
 */
CBLAuthDelegate::FlowState CBLAuthDelegate::handleRequestingCodePair() {
    ACSDK_DEBUG5(LX("handleRequestingCodePair"));
    /* #22313 setting NV parameter for error handlings */
    alexaClientSDK::avsCommon::utils::uci::SetparamUCI("shifu.conf", "ALEXA_REGISTRATION_STATUS", "not-registered");
    system("kill -SIGUSR2 `ps | grep Mobile_Agent | grep -v grep | awk '{print $1}'`");

	/// buffer to receive the data from MA
    char zmq_buffer[LWA_ZMQ_BUF_SIZE] = {0};
    /// json object to parse the received json data
    json_object *obj_reginfo;
    /// json variables to store parsed json data
    json_object *client_data;
    json_object *LWA_ClientId;
    json_object *LWA_AuthCode;
    json_object *LWA_CodeVerifier;
    json_object *LWA_RedirectUri;
    
    /// varible to store no of times to tried for authentication
    m_retryCount = 0;
    
    /* Initialize ZMQ if it is not initilized before */
    if(IsLwaZMQcontext != true)
    {
		LwaZmqContext = zmq_ctx_new ();
		LwaZmqResponder = zmq_socket (LwaZmqContext, ZMQ_REP);
		zmq_bind (LwaZmqResponder, "tcp://127.0.0.1:4640");
		IsLwaZMQcontext = true;
    }
	...
	if(isAVSinInitFirstTime == false){
		...
		syslog(LOG_DEBUG, "ALEXA: Sending AVS configured to LED module");
		isAVSinInitFirstTime = true;
	}
	avs_config.unlock();
	// #22267 End.

	ACSDK_DEBUG5(LX("handleRequestingCodePair").m("waiting for auth token from MA ------------->"));
	syslog(LOG_DEBUG, "ALEXA: waiting for auth token from MA ------------->");
	
	/* waiting here to get authentication parameters from mobile agent */
	zmq_recv (LwaZmqResponder, zmq_buffer, LWA_ZMQ_BUF_SIZE, 0);
	
	ACSDK_DEBUG5(LX("handleRequestingCodePair").m("received Auth code from MA"));
	syslog(LOG_DEBUG, "ALEXA: received Auth code from MA");
	
	/* parsing the received JSON data*/
	obj_reginfo = json_tokener_parse(zmq_buffer);
	
	/* extracting values from json object */
	json_object_object_get_ex(obj_reginfo, "client_data", &client_data);
	if(client_data!=NULL){
		json_object_object_get_ex(client_data, "amz_client_id", &LWA_ClientId);
		json_object_object_get_ex(client_data, "amz_auth_code", &LWA_AuthCode);
		json_object_object_get_ex(client_data, "amz_redirect_uri", &LWA_RedirectUri);
		json_object_object_get_ex(client_data, "amz_code_verifier", &LWA_CodeVerifier);
		
		if( (LWA_ClientId != NULL) 		|| 
			(LWA_AuthCode != NULL) 		|| 
			(LWA_RedirectUri != NULL) 	|| 
			(LWA_CodeVerifier != NULL) )
		{
			
			m_clientId = json_object_get_string(LWA_ClientId);
			m_authCode = json_object_get_string(LWA_AuthCode);
			m_redirectURI = json_object_get_string(LWA_RedirectUri);
			m_codeVerifier = json_object_get_string(LWA_CodeVerifier);
			
			/* this is the time after which auth code received from mobile agent expires. Amazon didn't provide the expire time for this, but we assumed it as 60 seconds.
			   Becasue our registration is completing with in 40 seconds.
			   
			   FIXME: Check with Amazon on the auth code expiry duration */
			m_codePairExpirationTime = std::chrono::steady_clock::now() + std::chrono::seconds(LWA_AUTH_CODE_EXPIRE_DURATION);
		}
		else
		{
			/* Returns the next action to perform. The data received from MA is in wrong format. Hence again wait in zmq receive to get proper data from MA */
			//TODO: Intimate to MA about the wrong data format. 
			return FlowState::REQUESTING_CODE_PAIR;
		}	
	}
	else
	...
	/* freeing memory occupied by json object as we are done with json data*/
	json_object_put(obj_reginfo);
	
	/* return the next acxtion to perform ie, requesting token from LWA using parameters received from MA */
    ...
    while (!isStopping()) {
        ...
        switch (result) {
            case AuthObserverInterface::Error::SUCCESS:
                ...
                /* #22313 #20994 LWA Registratio got successful. Send this status to mobile agent in json format */
                alexaClientSDK::avsCommon::utils::uci::SetparamUCI("shifu.conf", "ALEXA_REGISTRATION_STATUS", "registered");
                ACSDK_DEBUG5(LX("handleRequestingToken").m("LWA completed"));
                syslog(LOG_DEBUG, "ALEXA: LWA completed");
                JsonLWARespobj = json_object_new_object();
				json_object_object_add(JsonLWARespobj, "status_code", json_object_new_int(200));
                zmq_send (LwaZmqResponder, json_object_get_string(JsonLWARespobj), strlen(json_object_get_string(JsonLWARespobj)), 0);
                json_object_put(JsonLWARespobj);
                
                /* #20994 LWA Registratio got successful. SO closing ZMQ as we are done with registration*/
                if(IsLwaZMQcontext == true)
                {
					zmq_close (LwaZmqResponder);
					zmq_ctx_destroy (LwaZmqContext);
					IsLwaZMQcontext = false;
				}
				/* #20994 return next action to perform. ie, get refresh token*/
                return FlowState::REFRESHING_TOKEN;
            ...
            /* #20994 LWA Registration filed. Send this status to mobile agent in json format*/
            	ACSDK_DEBUG5(LX("handleRequestingToken").m("LWA failed - Again waiting for auth code from MA"));
            	syslog(LOG_DEBUG, "ALEXA: LWA failed - Again waiting for auth code from MA");
                JsonLWARespobj = json_object_new_object();
				json_object_object_add(JsonLWARespobj, "status_code", json_object_new_int(400));
                zmq_send (LwaZmqResponder, json_object_get_string(JsonLWARespobj), strlen(json_object_get_string(JsonLWARespobj)), 0);
                json_object_put(JsonLWARespobj);
                
            /* #20994 Return the next action to perform. LWA registration failed. Hence again wait in zmq receive to get new authcode from MA */
            	return FlowState::REQUESTING_CODE_PAIR;
...
/* #20994 changed the post data definition to get authentication tokens. This is as per the app based registration format. */
    const std::vector<std::pair<std::string, std::string>> postData = {{POST_KEY_GRANT_TYPE, POST_VALUE_AUTH_CODE},
                                                                       {POST_KEY_AUTH_CODE, m_authCode},
                                                                       {POST_KEY_REDIRECT_URI, m_redirectURI},
                                                                       {POST_KEY_CLIENT_ID, m_clientId},
...
AuthObserverInterface::Error CBLAuthDelegate::receiveCodePairResponse(const HTTPResponse& response) {
	...
	/* #20994 this will print auth code for code based registration. We are using app based registration, so commenting this. */
