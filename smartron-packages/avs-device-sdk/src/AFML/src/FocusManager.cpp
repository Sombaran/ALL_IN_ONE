 ...
 * */
std::shared_ptr<Channel> FocusManager::getHighestPriorityActiveChannel() {
    ACSDK_DEBUG9(LX("getHighestPriorityActiveChannel").d("action", "calling"));
    return getHighestPriorityActiveChannelLocked();
}
/**
 * Feature #21131 : End
 * */
...
/* Redmine 21139:start:Adding below function to know about  active channel
    This function returns true, if any channel is occupied otherwise returns false.
    This return value is used to trigger DND led pattern, when DND enabled from Alexa app.
*/
bool FocusManager::channelState()
 {
    auto channel = getHighestPriorityActiveChannelLocked();
    ACSDK_DEBUG9(LX("getHighestPriorityActiveChannel").d("channel", channel));
    if(channel)
    {
        ACSDK_DEBUG9(LX("getHighestPriorityActiveChannel").d("channel", "false"));
        return true;
    }
    else
        return false;
