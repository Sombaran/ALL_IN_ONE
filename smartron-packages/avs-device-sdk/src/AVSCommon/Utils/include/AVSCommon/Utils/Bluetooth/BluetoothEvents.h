...
enum class BluetoothEventType {
    ...
    // Redmine 21425: Start: Represents when an Volume changes has been received.
    SOURCE_VOLUME_RECEIVED
    // Redmine 21425: End
...
public:
    ...
    /**
     * Get @c Volume changes associated with the event
     * @return @c Volume changes associated with the event or 0 if not applicable
     */
    uint16_t get_shifu_source_volume() const;
...
protected:
    ...
    BluetoothEvent(
        ...
        uint16_t shifu_source_volume = 127);
...
private:
    ...
    /// @C Volume Change that is received
    uint16_t m_shifu_source_volume;
};
...
inline BluetoothEvent::BluetoothEvent(
    ...
    uint16_t shifu_source_volume) :
        ...
        m_shifu_source_volume{shifu_source_volume} {
}
...
inline uint16_t BluetoothEvent::get_shifu_source_volume()
    const {
    return m_shifu_source_volume;
}
...
// Redmine 21425: Start: Event indicating that an Volume Changes was received
class MediaStreamingVolumeChangedEvent : public BluetoothEvent{
public:
    explicit MediaStreamingVolumeChangedEvent(uint16_t volume);
};

inline MediaStreamingVolumeChangedEvent::MediaStreamingVolumeChangedEvent(
    uint16_t volume):
        BluetoothEvent(
            BluetoothEventType::SOURCE_VOLUME_RECEIVED, 
            nullptr, 
            avsCommon::sdkInterfaces::bluetooth::DeviceState::IDLE, 
            MediaStreamingState::IDLE, 
            nullptr,
