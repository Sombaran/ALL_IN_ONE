...
public:
    ...
    /**
     * This enum provides the type of the playing mediaplayer.
     */
    enum class Activemedia {
        /// No media playing
        AVS_MEDIA_NONE,
        /// Speaker type media is playing
        AVS_MEDIA_SPEAKER,
        /// Alert type media is playing
        AVS_MEDIA_ALERTS,
        /// Both speaker and alert type media is playing
        AVS_MEDIA_ALL
    };
...
/* #22033: Enum varibles to indicate the media player status.
*/
/**
 * Write a @c Activemedia value to an @c ostream as a string.
 *
 * @param stream The stream to write the value to.
 * @param type The type value to write to the @c ostream as a string.
 * @return The @c ostream that was passed in and written to.
 */
inline std::ostream& operator<<(std::ostream& stream, SpeakerInterface::Activemedia activemedia) {
    switch (activemedia) {
        case SpeakerInterface::Activemedia::AVS_MEDIA_NONE:
            stream << "AVS_MEDIA_NONE";
            return stream;
        case SpeakerInterface::Activemedia::AVS_MEDIA_SPEAKER:
            stream << "AVS_MEDIA_SPEAKER";
            return stream;
		case SpeakerInterface::Activemedia::AVS_MEDIA_ALERTS:
            stream << "AVS_MEDIA_ALERTS";
            return stream;
        case SpeakerInterface::Activemedia::AVS_MEDIA_ALL:
            stream << "AVS_MEDIA_ALL";
            return stream;    
    }
    stream << "UNKNOWN";
    return stream;
