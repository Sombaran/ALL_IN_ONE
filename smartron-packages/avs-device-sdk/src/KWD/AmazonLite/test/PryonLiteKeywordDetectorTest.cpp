...
/// Tests that we don't get back a valid detector when empty pathToModelFile is
/// passed in.
TEST_F(PryonLiteKeywordTest, emptyPathToModelFile) {
    auto audioBuffer = std::make_shared<avsCommon::avs::AudioInputStream::Buffer>(AUDIO_STREAM_BUFFER_SIZE);
    auto audioSDS = avsCommon::avs::AudioInputStream::create(audioBuffer, SDS_DATA_WORD_SIZE, SDS_DATA_READERS);
    std::shared_ptr<AudioInputStream> audioStream = std::move(audioSDS);

    std::string pathToModelFile = "";
    auto detector = PryonLiteKeywordDetector::create(audioStream, compatibleAudioFormat, {keyWordObserver1}, {stateObserver}, pathToModelFile);
    ASSERT_FALSE(detector);
}
