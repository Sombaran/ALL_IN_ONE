...
#ifdef AMAZONLITE_USE_EMBEDDED_MODEL
...
/// The minimum allowed detect threshold.
static const int32_t MINIMUM_DETECT_THRESHOLD = 1;

/// The maximum allowed detect threshold.
static const int32_t MAXIMUM_DETECT_THRESHOLD = 1000;
...
#ifdef AMAZONLITE_USE_EMBEDDED_MODEL
    return doCreate( stream,
             audioFormat,
             keyWordObservers,
             KeyWordDetectorStateObservers,
             std::string(),
             detectThreshold,
             voiceActivityDetectionCallback
         );
#else
    ACSDK_ERROR(LX("createFailed").d("reason", "This create method requires the adapter to be complied along with a key word detector model."));
    return nullptr;
#endif
}
...
std::unique_ptr<PryonLiteKeywordDetector> PryonLiteKeywordDetector::create(
    ...
    if (pathToModelFile.empty()) {
        ACSDK_ERROR(LX("createFailed").d("reason", "pathToModelFile not valid"));
        return nullptr;
    }
    /**
     ...
     * 
     ***/
     ACSDK_DEBUG9(LX("custom threshold from smartron").d("detectThreshold", detectThreshold));
    return doCreate( stream,
        audioFormat,
        keyWordObservers,
        KeyWordDetectorStateObservers,
...
std::unique_ptr<PryonLiteKeywordDetector> PryonLiteKeywordDetector::doCreate(
        const std::shared_ptr<avsCommon::avs::AudioInputStream>& stream,
        avsCommon::utils::AudioFormat audioFormat,
        const std::unordered_set<std::shared_ptr<avsCommon::sdkInterfaces::KeyWordObserverInterface>>& keyWordObservers,
        const std::unordered_set<std::shared_ptr<avsCommon::sdkInterfaces::KeyWordDetectorStateObserverInterface>>&
    ...
    if (detectThreshold < MINIMUM_DETECT_THRESHOLD || detectThreshold > MAXIMUM_DETECT_THRESHOLD) {
...
#ifdef AMAZONLITE_USE_EMBEDDED_MODEL
        ACSDK_INFO(LX(__func__).m("usingEmbeddedKeywordModel"));
        config.model = static_cast<void*>(prlBinaryModelData);
        config.sizeofModel = prlBinaryModelLen;
#else
        ACSDK_ERROR(LX("initFailed").d("reason", "embeddedModelFileNotAvailable"));
        return false;
#endif
    } else {
