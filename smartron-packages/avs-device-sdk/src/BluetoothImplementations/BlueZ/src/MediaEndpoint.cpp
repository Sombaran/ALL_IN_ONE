...
void MediaEndpoint::onMediaTransportStateChanged(
    ...
    // Redmine 21298: Start: As MediaPlayer has Majorly 2 States, Playing and Paused/stopped, Pending is changed to Active
    if (avsCommon::utils::bluetooth::MediaStreamingState::ACTIVE == newState) {
    // Redmine 21298: End
        ...
        // Redmine 21298: Start: When Media is Idle/paused/stopped, Stream should be Release, Else it will wait until the stack time for release.
        std::shared_ptr<DBusProxy> transportProxy =
            DBusProxy::create(BlueZConstants::BLUEZ_MEDIATRANSPORT_INTERFACE, m_streamingDevicePath);
        ...
        GUnixFDList* fdList = nullptr;
        ManagedGVariant transportDetails =
            transportProxy->callMethodWithFDList("Release", nullptr, &fdList, error.toOutputParameter());
        if (error.hasError()) {
            ACSDK_ERROR(LX("onMediaTransportStateChangedFailed")
                            .d("reason", "Failed to release media stream")
                            .d("error", error.getMessage()));
            return;
        } 
        // Redmine 21298: End
