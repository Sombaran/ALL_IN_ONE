...
public:
	...
	/// Thread to check internet status periodically
    ...
    /** #21219
    * get priority value of active channel
    */
    unsigned int getActiveChannelPriority();
	 ...
	 * */
	std::future<bool> notifyOfActionButton(
    ...
    ~DefaultClient();
	  /**
     * BUG:21972
     * signal handlers to know the status of the connnection from External module(internet_check script)
     */
	 static void network_connected(int signal_value);
	 static void network_disconnected(int signal_value);
...
private:
	...
	// Redmine 21700: start: After Deregistration,if user press action button, no internet announcement observed
	/// Flag to indicate whether network is up or down.
     ...
     */
	static int m_isNetworkConnected;
	int m_present_AVSNetworkConnected = UNKNOWN;
	// Redmine 21700: end.
	 ...
	 * Changed by Smartron India Pvt. Ltd.
	 * */
	/// Flag to indicate that media is paused when user utters Alexa in case of no internet
	bool m_isMediaPaused = false;
	/**
	 * Bug #21360 : End
	 * */

	/// Network detection thread
	std::thread m_networkDetectionThread;
