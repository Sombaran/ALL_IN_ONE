...
typedef struct lssdp_packet {
	...
	char            device_id   [LSSDP_FIELD_LEN];
	char            device_mode [LSSDP_FIELD_LEN];
...
int lssdp_socket_create(lssdp_ctx * lssdp) {
	...
	// set socket to be non-blocking. All of the sockets for the incoming connections will also be nonblocking since they will 
...
int lssdp_socket_read(lssdp_ctx * lssdp) {
	...
	if (lssdp->debug) {
		lssdp_info("RECV <- %-8s   %-28s  %s\n", packet.method, packet.location, packet.device_id);
	}
...
int lssdp_send_notify(lssdp_ctx * lssdp) {
	...
	for (i = 0; i < lssdp->interface_num; i++) {
		...
		snprintf(notify, sizeof(notify),
			...
			lssdp->header.device_id,                    // DEVICE_ID    (addtional field)
			lssdp->header.device_mode                   // DEVICE_MODE (addtional field)
		);
		// send NOTIFY
...
static int parse_field_line(const char * data, size_t start, size_t end, lssdp_packet * packet) {
	...
	if (field_len == strlen("device_id") && strncasecmp(field, "device_id", field_len) == 0) {
		memcpy(packet->device_id, value, value_len < LSSDP_FIELD_LEN ? value_len : LSSDP_FIELD_LEN - 1);
		return 0;
	}

	if (field_len == strlen("device_mode") && strncasecmp(field, "device_mode", field_len) == 0) {
		memcpy(packet->device_mode, value, value_len < LSSDP_FIELD_LEN ? value_len : LSSDP_FIELD_LEN - 1);
		return 0;
	}
...
static int neighbor_list_add(lssdp_ctx * lssdp, const lssdp_packet packet) {
	...
	for (nbr = lssdp->neighbor_list; nbr != NULL; last_nbr = nbr, nbr = nbr->next) {
		...
		// device_id
		if (strcmp(nbr->device_id, packet.device_id) != 0) {
			lssdp_debug("neighbor device_id is changed. (%s -> %s)\n", nbr->device_id, packet.device_id);
			memcpy(nbr->device_id, packet.device_id, LSSDP_FIELD_LEN);
			is_changed = true;
		}

		// device mode
		if (strcmp(nbr->device_mode, packet.device_mode) != 0) {
			lssdp_debug("neighbor device_mode is changed. (%s -> %s)\n", nbr->device_mode, packet.device_mode);
			memcpy(nbr->device_mode, packet.device_mode, LSSDP_FIELD_LEN);
			is_changed = true;
		}
